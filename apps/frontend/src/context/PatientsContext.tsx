...
import { Patient, patientsSeed } from "../mock/patients";
...
type PatientsContextValue = {
  ...
  updatePatient: (patientId: string, changes: Partial<Patient>) => void;
  selectedCheckinLetter: string | null;
  setSelectedCheckinLetter: (letter: string | null) => void;
  selectedPaymentLetter: string | null;
  setSelectedPaymentLetter: (letter: string | null) => void;
  findPatientByPhone: (phone: string) => Patient | null;
  resetAll: () => void;
  clearSelection: () => void;
  selectedPaymentMethod: PaymentMethod | null;
  setSelectedPaymentMethod: (method: PaymentMethod | null) => void;
  selectedInstallments: string | null;
  setSelectedInstallments: (installments: string | null) => void;
  supportPhone: string;
  registerSupportPhone: (phone: string) => void;
  chatMessages: ChatMessage[];
  addChatMessage: (author: ChatMessage["author"], text: string) => void;
  sendWhatsappConfirmation: (
    patient: Patient,
    scenario: "pagamento" | "checkin",
  ) => void;
  addPatient: (name: string) => Patient;
};

const PatientsContext = createContext<PatientsContextValue | undefined>(
  undefined,
);
...
export function PatientsProvider({ children }: { children: ReactNode }) {
  ...
  const [selectedCheckinLetter, setSelectedCheckinLetter] = useState<
    string | null
  >(null);
  const [selectedPaymentLetter, setSelectedPaymentLetter] = useState<
    string | null
  >(null);
  ...
  const lastWhatsappRef = useRef<string | null>(null);
  ...
  const updatePatient = useCallback(
    (patientId: string, changes: Partial<Patient>) => {
      setPatients((prev) =>
        prev.map((patient) =>
  ...
  const findPatientByPhone = useCallback(
    (phone: string) => {
      ...
      return (
        patients.find(
          (patient) => normalizePhone(patient.telefone) === normalized,
        ) ?? null
      );
    },
    [patients],
  ...
  const addChatMessage = useCallback(
    (author: ChatMessage["author"], text: string) => {
  ...
  const registerSupportPhone = useCallback(
    (phone: string) => {
      const normalized = normalizePhone(phone);
      ...
      addChatMessage(
        "assistant",
        `Perfeito! Vou usar ${formatPhone(normalized)} para enviar atualizações.`,
      );
    },
    [addChatMessage],
  );
...
const addPatientCallback = useCallback(
  ...
  const sendWhatsappConfirmation = useCallback(
    (patient: Patient, scenario: "pagamento" | "checkin") => {
      if (!patient) return;
      const label =
        scenario === "pagamento"
      ...
      if (!supportPhone) {
        addChatMessage(
          "assistant",
          "Para te avisar por WhatsApp, compartilhe o número no campo ao lado do totem.",
        );
        return;
      }
      ...
      addChatMessage(
        "assistant",
        `Enviei um WhatsApp para ${formatPhone(
          supportPhone,
        )} avisando que ${label} ${patient.nomeCompleto}.`,
      );
    },
